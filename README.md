<p align="center">
  <img src="public/logo-icon.png" alt="Confidia Logo" width="150">
</p>

<h1 align="center">Confidia</h1>

<p align="center"><strong>Use AI without leaking client data.</strong></p>

[![License: AGPL-3.0](https://img.shields.io/badge/License-AGPL--3.0-blue.svg)](LICENSE)

Confidia is an open-source document anonymizer that strips personally identifiable information (PII) before you share documents with AI services - and restores the original names in the AI's response.

Everything runs in your browser. No server, no API calls, no data leaves your machine.

![Confidia Demo](docs/redact.gif)
...
2. **Redact** - Confidia detects names, emails, phone numbers, addresses, and other PII using a local ML model + regex patterns
3. **Copy** the anonymized text and paste it into any AI service (ChatGPT, Claude, Gemini, etc.) - or **download** the redacted document
4. **Restore** - paste the AI's response back into Confidia to replace placeholders with the original names
...
- **Entity propagation** - when a name or company is detected once, Confidia automatically finds all other occurrences throughout the document
- **Round-trip de-anonymization** - paste the AI's response back in and Confidia restores the original names automatically
...
- **9 languages** - Arabic, French, English, Polish, German, Spanish, Portuguese, Swedish, Norwegian
...
Confidia doesn't ask you to trust a server, a company, or a privacy policy. It's built so you don't have to trust anyone.
...
- **No tracking, no analytics, no telemetry.** Confidia doesn't know who you are, what you paste, or how often you use it.
...
- **Works offline after first load.** Once the model is cached, you can disconnect from the internet and Confidia keeps working — anonymization runs entirely in WebAssembly.

### Model caching and offline use

After a model is downloaded for the first time, Confidia stores it in the browser's [Cache Storage](https://developer.mozilla.org/en-US/docs/Web/API/Cache) under the `be-anonymized-models` cache. On subsequent visits the model loads from local storage instead of re-downloading from HuggingFace, so you can use Confidia fully offline.

Caching is **best-effort**. If your browser refuses to cache the model — for example because the per-origin storage quota is exceeded, you're using an Incognito/Private window with restricted quota, or the model file is larger than the browser allows for a single Cache entry — Confidia still loads the model into memory and works normally for the current session. The next visit will simply re-download it instead of using the cache.

The BardS.ai EU PII model (~279 MB) is most likely to hit quota limits, especially in Incognito mode. GLiNER PII Edge (~65 MB) caches reliably almost everywhere. To force a re-download (e.g. after a model update), open DevTools → Application → Cache Storage → delete the `be-anonymized-models` cache.
